...
echo -e "\n${YELLOW}Testing name resolution and connectivity:${NC}"
for host in k8s-master-01 k8s-worker-01 k8s-worker-02; do
    # First, check if the hostname can be resolved
    if getent hosts "$host" > /dev/null; then
        # If it resolves, then check for network reachability
        if ping -c 1 -W 1 "$host" > /dev/null 2>&1; then
            echo -e "${GREEN}✓${NC} $host: Resolved and Reachable"
        else
            echo -e "${YELLOW}✓${NC} $host: Resolved (but not reachable yet)"
        fi
    else
        # If it doesn't resolve, report the failure
